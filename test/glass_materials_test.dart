...
void main() {
  test('theme changes glass tint without changing optical recipes', () {
    ...
    final changed = original.copyWith(
      composerSurface: const Color.from(alpha: .6, red: .1, green: .2, blue: .3),
    ...
    expect(
      ...
      Color.lerp(original.commandPaletteSurface, changed.commandPaletteSurface, .5),
    );
  });

  testWidgets('navigation surface takes explicit color, theme token when unset', (
    tester,
  ) async {
    const child = Column(
      children: [
    ...
    await tester.pumpWidget(
      const MaterialApp(home: Scaffold(body: child)),
    );
      ...
      MaterialApp(
        home: GlassTheme(
          data: const GlassThemeData(
            navigationSurface: Color.from(alpha: .75, red: 0, green: 1, blue: 0),
    ...
    expect(
      ...
      const Color(0xFF4B0082).toARGB32(),
    );
    expect(
